...
def handle_menu_callback(bot, call, menu_key):
    """Центральный диспетчер callback-запросов меню"""
    try:
        # Проверяем, что callback ещё не обработан
        if hasattr(call, '_processed') and call._processed:
        ...
        if not menu:
            ...
            if call.data.startswith("swap_dej_"):
                ...
                return
            elif call.data.startswith(("confirm_swap_", "reject_swap_")):
                from .swap_dej_handler import process_swap_confirmation
                from_user_id = int(call.data.split("_")[2])
                is_confirmed = call.data.startswith("confirm_swap_")
                process_swap_confirmation(bot, call, from_user_id, is_confirmed)
                return
